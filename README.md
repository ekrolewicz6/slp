...
| What does full FluencyBank transcript access show about stuttering recovery? | `outputs/fluencybank_full_recovery_model/summary.md` |
| Which FluencyBank media can currently be streamed? | `outputs/fluencybank_media_access_probe/summary.md` |
| Which external datasets and papers are now downloaded or gated? | `outputs/data_access_scan/summary.md` |
| Which clinical papers should guide the research plan? | `outputs/clinical_literature_review/summary.md` |
| Does sentence repetition behave as a structured DLD state probe? | `outputs/fiveash_sentence_repetition_pilot/summary.md` |
| Can repeated grammar-treatment probes model target-specific DLD response? | `outputs/calder_repeated_probe_pilot/summary.md` |
| What should we request from SCALES? | `docs/scales_access_packet.md` |
...
FluencyBank transcript access is now fully unblocked under the current credential. The download inventory contains all 1,999 `.cha` transcripts in the TalkBankDB export, including 1,154 formerly password-gated transcripts from IISRP, IISRP-new, Wagovich, Ratner, Maxfield, Tellis, and Sawyer. Purdue should be cited as the FluencyBank English Purdue Corpus by Anne Smith, Christine Weber, Amanda Hampton Wray, Bridge Walsh, and Evan Usler, DOI `10.21415/P2JB-CA45`. The full recovery audit parsed 1,922 transcripts and found 253 recovery-labelled CWS participants with usable transcript features. The best earliest-session signal was language-level rather than disfluency-only (AUC 0.717), but early movement did not improve the model among 152 multi-session labelled participants: movement-only AUC was 0.421, and first-plus-movement changed AUC by -0.010 with bootstrap 95% CI [-0.118, 0.083]. Leave-corpus-out tests were weak. This is a valuable negative result: transcript-only early movement is not yet the stuttering recovery discovery we hoped for.

The FluencyBank media probe shows why acoustic stuttering recovery is still gated. Twelve of 17 sampled corpora had at least one accessible media sample, but IISRP, IISRP-new, Wagovich, and Purdue did not stream in the current environment; Purdue's TalkBank page says audio is not available. That means the recovery-labelled stuttering corpora remain transcript-only for now, while Ratner, UMD-CMU, and several teaching/fluency corpora are possible acoustic feasibility targets.

The 2026-05-01 clinical literature scan led to three completed DLD/structured-task outputs. Fiveash, Ladanyi, Camici, Chidiac, Bush, Canette, Bedoin, Gordon, and Tillmann (2023), DOI `10.1038/s41539-023-00170-1`, provide open OSF sentence-repetition data. Our secondary analysis found that sentence-repetition level is a strong DLD-vs-TD state probe in this small sample (task-only leave-one-child-out AUC 0.944), while rhythm response alone is much weaker (AUC 0.652) and not clearly DLD-specific. Calder, Claessen, Ebbels, and Leitão (2020), DOI `10.1044/2019_LSHSS-19-00060`, provide ASHA Figshare repeated-probe supplements. Parsing those tables produced 1,638 probe rows and showed strong target-specific expressive maintenance gains for untrained past tense relative to a possessive-s control. SCALES Study 8968 is now packaged as the highest-value gated DLD access request, with a variable map and six-model analysis plan in `docs/scales_access_packet.md`. Ward, Bannard, Norbury, and Polisenska (2026), DOI `10.1044/2025_JSLHR-25-00058`, further supports sentence repetition as a robust DLD marker in SCALES while reinforcing that additional assessment is needed for clinical decisions.

The next high-value work is blocked on fewer external inputs than before: BA Web integration details, SLP review of the report packets, access to raw transcript/audio or session-level EMT-SF dose/target data, UK Data Service access for SCALES/Manchester, and FluencyBank media access/quality checks before acoustic recovery modeling.
...
- FluencyBank transcripts stored locally under gitignored `data/raw/fluencybank/`; the current credential downloads all 1,999 `.cha` transcripts in the TalkBankDB export, including Purdue and the formerly password-gated IISRP, IISRP-new, Wagovich, Ratner, Maxfield, Tellis, and Sawyer corpora; source citation for Purdue: FluencyBank English Purdue Corpus, Smith, Anne; Weber, Christine; Hampton Wray, Amanda; Walsh, Bridge; Usler, Evan, DOI `10.21415/P2JB-CA45`
